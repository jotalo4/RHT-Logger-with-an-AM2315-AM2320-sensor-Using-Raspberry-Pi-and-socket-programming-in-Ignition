...
#modified by: Orukotan, Ayomikun Samuel 
...
class THP_Logger(Frame):
    ...
    def timer_call_back(self):
        ...
        (_temperature , _humidity , _fahrenheit ) = get_data()

        #update user interface
        self.chrtTemperature.add_point(x,_temperature )
        self.chrtHumidity.add_point(x, _humidity)
        self.chrtPressure.add_point(x, _fahrenheit)

        #log data to csv file if logging is enabled
        if self.isLogToFile:
            _index = self.log_entry_index
            self.data_logger.log(entry_index=_index, temperature=_temperature, humidity=_humidity, fahrenheit=_fahrenheit)
